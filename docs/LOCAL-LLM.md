...
## 1ו. No LM Studio, no GPU — llama.cpp on a CPU (v2.45)

The battery was run once on a machine that has neither: a 4-vCPU / 16 GB Linux VM, `llama-server` (build b11046) with Google's `gemma-4-E4B-it-qat-q4_0` GGUF, prompt read at ≈ 55–65 tokens/s and words written at ≈ 7 tokens/s. The copilot score on that box — **17/39** (local ×3) and **7/13** (relay ×1) at the full 32K tier — is in [`SCORECARD-2.45-copilot-battery.md`](SCORECARD-2.45-copilot-battery.md); §5 below still holds the 5090 family table. Three things about the *runtime* came out of it, and two of them are invisible on a 5090.

**The probe route.** The copilot reads the window from `GET /api/v0/models` (§1א), which only LM Studio serves. Every other runtime is planned on the advisory 24K and pinned to the **compact** tier — by the contract's rule, an unmeasured window never promotes to the full dictionary — so a battery run straight at `llama-server` measures the compact tier, not the 32K rows in §5. `scripts/lmstudio-probe-shim.js` answers that one route with the window the runtime was really loaded with and forwards everything else byte for byte (nothing cached, rewritten or retried). It is a harness, not a product path: the CMS's own answer to "my runtime is not LM Studio" is still the compact tier and the window chip that says so.

```bash
./llama-server -m gemma-4-E4B_q4_0-it.gguf -c 32768 --host 127.0.0.1 --port 8080 \
  -t 4 -tb 4 --parallel 1 --swa-full --cache-reuse 256 --jinja --reasoning off -fa on --alias gemma-4-E4B-it-qat-q4_0
node scripts/lmstudio-probe-shim.js --upstream=http://127.0.0.1:8080 --port=1234 --model=gemma-4-E4B-it-qat-q4_0 --ctx=32768
LOCAL_LLM_BASE=http://127.0.0.1:1234/v1 LOCAL_LLM_MODEL=gemma-4-E4B-it-qat-q4_0 node scripts/battery-copilot.js --runs=3
```

**The briefing must be the same bytes on every turn.** At 7 tokens/s a hello cost **6½ minutes**, and so did every turn after it: llama.cpp chose the slot by prefix similarity and reported `f_sim_best = 0.188` — the cached prompt and the new one parted ≈ 19 % in. `buildDictionary()` wrote `new Date().toISOString()` into the header line of the syntax dictionary, which rides ≈ 22 % into the copilot's ≈ 17K-token system prompt; the briefing is rebuilt on every owner turn, so every turn was a *new* prefix and the ≈ 13K tokens after the stamp were read again — ≈ 3 s on a 5090, where nobody noticed, ≈ 4–5 minutes on a CPU or a laptop. The stamp is now the package version (`DICTIONARY_STAMP`, `src/pzn/syntax-dictionary.js`), the briefing is byte-identical between turns (checked with two dumps, `cmp`), and once the prefix is warm the battery's T1 drops to **52–82 s** on runs 2–3 (run 1's first hello stays **396 s** for the cold ingest). This is the product fix of the run; the two lines above and below it are the runtime's.

**`--swa-full`, or every new conversation reads the briefing again.** Gemma 4 mixes sliding-window attention layers with global ones, and llama.cpp's default SWA cache keeps only the window for those layers (`cache_reuse is not supported by this context` in the load log). A turn that *extends* the conversation reuses the cache; a turn whose prompt parts from the cached one more than ≈ 500 tokens before its end cannot roll the SWA layers back and is read from the first token — which is every **new** conversation after one that ran a few turns (the battery's T4 after T3: `f_keep = 0.945`, and still 17,468 tokens re-read, **321 s** for a 122-token answer). With `--swa-full` the SWA layers keep the whole window: the same T4 read 770 tokens and answered in **32 s**. The price is memory — the KV cache of the SWA layers at the full window (≈ 2.5 GB more for the E4B at 32K, 9.3 GB RSS in all) — and the CPU box had it. On a 5090 the re-read is three seconds; on anything slower this flag is the difference between a copilot and a coffee break. LM Studio has no such switch; its GPU hides the cost.
...
**6. The same write again, after ✓ (v2.60).** Told its edit had landed, a model's closing turn sometimes proposes the identical edit on a second card (live on the English site, 2026-09-24; once in 24 measured approvals). A closing-turn `edit_page` on the page the turn just saved, whose document says the same page as the saved draft (ids and whitespace aside), is answered — *already saved, say so in words* — and never shown; the owner reads one line that the double proposal was skipped. A proposal that differs is a new change and gets its card.
...
| the Hebrew site rebuilt — a theme, twelve pages and a menu, one sentence each | **the LIVE Hebrew site, Ben's own Chrome, his installed Bridge** (v2.63 → v2.64, 2026-09-24) | the theme from a Hebrew brief through the theme-designer card: 168 s, 9,563 tokens, all seven knobs asked for came back (`header-layout=centered`, `drawer=overlay`, `menu-current=pill`, Heebo + Suez One, the palette) · 12 pages (9 new, 3 rewritten): a card in 87–259 s — 130 s average when no door sent it back; 3 of 12 went back once (a link to a page not built yet; `E_NOT_CONTAINER` ×6 and ×2) — a new page's draft 23–30 s after ✓, an edit's within a second; 293 modules · 12/12 `lang="he" dir="rtl"` · 0 template tokens · the menu in one sentence: 15 + 3 links in two groups, 624 of 880 px, applied 28 s after ✓, `PAGES_LOST` not raised because the sentence said הסר · 2 owner turns per page | the field found the drawer wrap (v2.64) and taught the order: build the page you link to first, and swap live store modules for words on a site whose store is closed |
| six pages rebuilt, one sentence each | **the LIVE English site, Ben's own Chrome, his installed Bridge** (v2.59) | a card in 35–70 s (49 s average), the draft 30–35 s after the click, 2 owner turns each · 211 modules · 36 pictures, all real library paths · 0 Hebrew letters · every page ltr · the gallery arranged as an exhibition (three galleries, a carousel, a before/after) · one small edit landed in 35 s with the rest untouched · one edit of a 67-module page CUT at 4,096 tokens → the v2.59 budget fix | the site the owner has, rebuilt the owner's way |
| the cut edit, again, on v2.59 | **the LIVE English site, Ben's own Chrome, his installed Bridge** | the same "add one paragraph, touch nothing else" on the same 80-module dreams page: the card in **140 s**, the draft **35 s** after the click, 80 → 82 modules, every fold, stat, table and picture kept, the paragraph word for word · then the model's closing turn proposed the SAME edit again on a second card (refused, nothing written) — seen once in 24 measured approvals too (E3). Two cosmetics on the way: the turn clock read "wrote nothing in 45 s" while the model was streaming a tool call, and the CUT error's old sentence blamed the window | the field proof of the reply budget that follows the page |
| two edits on v2.60, "let's see it's not annoying" | **the LIVE English site, Ben's own Chrome, his installed Bridge** | "add a short honest note under the title": the card in **116 s**, the draft saved with the click (0.3 s), the closing words **7 s** later, +184 characters, the rest of the 83 modules untouched · "change the last line": the card in **117 s** — for a page with NO change in it (the draft the same 17,849 characters before and after ✓) · then the closing turn re-sent that page three times, the v2.60 door dropped each copy (the notice line, no card) but answered the model each time, and the fourth copy stalled: a red "check LM Studio" **8 min 6 s** after ✓ · the clock read "wrote nothing in 45 s" while every copy streamed | the four habits v2.61 removes: a repeat ends the turn, a saved draft never ends red, an unchanged page is not a card, the clock knows a tool call |
| the same page on v2.61, two more sentences | **the LIVE English site, Ben's own Chrome, his installed Bridge** | "replace the last paragraph (quoted) with one line": the clock at 1:31 read *writing the page into the edit, it arrives whole* (no more "check LM Studio"), the card in **105 s**, the draft saved with the click, the closing words **6 s** later, the input free — the old paragraph gone, the new line there, the first edit's note kept, 83 modules, 0 Hebrew · "make that closing line a little warmer": answered in **6 s** with the page's own memo line and "I've updated the closing line…" — no call, nothing written, the draft unchanged over the API | v2.61 holds; the claim without a write became v2.62 |
| the same sentence on v2.62 | **the LIVE English site, Ben's own Chrome, his installed Bridge** | this time the model read the page first and did it: the card in **106 s**, the draft saved with the click, "I've updated the last line…" **6 s** later — true this time, so the claim door stayed quiet — and over the API the last line reads *Fourteen dreams, one Mac, no cloud. Thank you to the owner for typing them.*, 83 modules, 0 Hebrew · five owner sentences on one page across three versions in one night: three landed as asked, two became doors | the habits are the model's and stochastic; the smokes are the proof the doors hold, the field shows what an owner sees |
| one English dream | **the LIVE English site, Ben's own Chrome, his installed Bridge** | the card in **36 s**, the draft **31 s** after the click · 2 owner turns · `lang="en" dir="ltr"` · 22 modules · all five of her facts (Thursday, 21:00, Jaffa, free, a friend) · 0 Hebrew letters · 3 pictures, all real paths from the media library | the capstone: v2.58 deployed, the copilot told the site is English, the extension the owner actually has |

"Owner turns" count what a person typed or clicked: the wish, a shrug when the model asked ("you decide"), a yes to a plan, the approval. The Mac's dreams page on the English site is generated from these JSONs. The capstone draft (`launch-party`) was left on the site for its owner to publish or delete.
