...
**On the Mac (MLX):** `bash scripts/mlx-survey.sh` runs the whole dreams-first survey for a fixed list of MLX builds — download, load at 32K `--parallel 1`, D1–D14, the theme and page dreams, unload, one row per model; orders and cluster rules in `eval/battery/RUN-ON-MAC.md`. The battery refuses to start when something already answers on its port (an interrupted run leaves its server behind, and the next run would talk to THAT site while judging a fresh one).
