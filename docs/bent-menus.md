...
**A printed menu from a model that READ the menus (v2.45).** Not every printed document is the weaker mode. Gemma 4 12B calls `read_menus`, regroups the menu well — and then prints the `<bent-menus>` document in a fence instead of calling `organize_menu` (the copilot battery, T9, 2/2); the work is done and the owner has nothing to press. When `read_menus` ran **this turn** and `organize_menu` is declared, the tool loop adopts the printed document as the call it was meant to be (`ai.adoptPrintedDocument`): the same preflight — the door's refusals and `PAGES_LOST` go back to the model under the adopted call's id — and the same approval card with its canvas. A menu from a model that never read the menus is left exactly as above: `organize_menu` replaces whole menus, and nobody should be asked to approve a menu written from imagination. Pages follow the same rule — see [LOCAL-LLM.md](LOCAL-LLM.md) §3ב.
