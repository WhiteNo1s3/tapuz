...
| **Leads land in the CMS — and get worked** | forms inbox (תיבת פניות) with honeypot + rate-limit, dashboard tile, **CSV export** (v0.87), **email notification on every new lead** (v0.94), **a real lead pipeline** — status (new→contacted→qualified→won/lost) + private notes (v1.00), **deal value + follow-up dates** with a pipeline-value summary and an overdue/due-today tile, right in the inbox (v1.12), **CRM progressive cards + interests + segment mail** (v1.92–1.98), **sales tasks (משימות) on the person** — call/email/follow-up with due board, HubSpot's "next action" without the SaaS meter (v2.00) | Contact Form 7 + a storage plugin (no pipeline, no forecast without a CRM plugin) | webform modules | yes — their strongest suit |
