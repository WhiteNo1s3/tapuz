...
| | |
|---|---|
| Package / ROADMAP | **`2.49.0-alpha`** — the Version Log's top row must name it (`smoke-version`) |
| Main tip | the v2.49 merge — `git log -1 origin/main` |
...
| 2ט | A thinking model burns the answer's budget | Found by the survey (Muse-Glimmer 30B, 2026-09-20): LM Studio ignores `reasoning_effort: "none"` for it, so every reply starts with 2,000–4,000 reasoning tokens. The organizer's run budget is `maxTokens: 2048` (`src/injections/menu-organizer.js`) → `finish=length`, empty `content`, and the owner reads "הספק החזיר תשובה ריקה" (25 of 27 runs); the pasted-document dream dies the same way in the copilot (`EMPTY_REPLY`). Probed: the same organizer prompt with 6,000 tokens returns a valid `<bent-menus>`. The fix is small and ours: when a reply is empty AND `finish_reason === "length"` AND reasoning tokens were spent, retry ONCE with a larger budget (window permitting), and if it is still empty say the truth — "the model spent its whole answer thinking" — instead of "empty reply". Until then Muse cannot be recommended, although it is the strongest newcomer (dreams 12/14, T-battery 26/26). |
