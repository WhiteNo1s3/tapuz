...
> **Last updated:** 2026-09-24 — **v2.65-alpha** (the overlay drawer's close button is pinned to the panel's corner while it is open, and the panel's list starts at its top — both seen under the centered masthead on the live Hebrew site), before it **v2.64-alpha** (the Hebrew site rebuilt through the Bridge in Ben's Chrome: a theme from a brief, twelve pages and a menu one sentence each, numbers in `docs/LOCAL-LLM.md` §5; the overlay drawer scrolls instead of wrapping a long list into a second column), before it **v2.63-alpha** (the header moves: `menu=top|side|side-end|bottom`, `header-layout=bar|centered|floating`, `drawer=inline|overlay` on the theme document, CSS only; the floating pill is the English site's face; next is the motion — `header-scroll`, a progress line, `stagger`, a sticky module — which needs the first script a theme ships), before it **v2.62-alpha** (words that claim a change are not a change: a reply with no write that says "I've updated…" or speaks the page's own memo line is sent back once, then told beside its words that nothing changed — the fourth habit seen live on the dreams page), before it **v2.61-alpha** (after ✓ the owner is done: a repeated closing-turn proposal ends the turn instead of feeding the model again; nothing after the draft landed is an error; an edit that changes nothing is not a card; the clock knows a tool call is streaming — all four seen live on the dreams page in the owner's Chrome), before it **v2.60-alpha** (a closing turn that proposes the write it just had approved is answered, not shown — the second card is gone), before it **v2.59-alpha** (the English site rebuilt page by page through the Bridge in the owner's own Chrome, 49 s to a card on average and every picture real; the reply budget now follows the page an edit rewrites, and a cut document gets one more call; the cornerstone guide `docs/BUILD-AT-HOME.md`), before it **v2.58-alpha** (an English site is English: `config.language en` makes new pages ltr, the chrome English and the copilot an English writer, and the settings switch turns the pages around — `docs/hebrew-rtl.md` § An English site; the Bridge is proved by the extension itself typed into the copilot's screen — `battery-copilot.js --courier=extension`, `docs/LOCAL-LLM.md` §3ג; the battery refuses a run answered by a model other than the one it names; the English track E1–E10; the second Hostinger site rebuilt as the English face of Tapuziel), before it **v2.57-alpha** (Geppetto's SVG door: a design exported as SVG from Figma — or any vector tool — becomes a living Tapuziel site, and the designer's own **layer names are read as hints** that never override the geometry; a free Figma account cannot publish a Site, but it can export this — `docs/bent-geppetto.md`), before it **v2.56-alpha** (Geppetto: a published Canva site or Figma Site, pasted as one address, becomes a living Tapuziel site — BenTML pages of real modules, a menu, a `<bent-theme>`, the media at home — previewed, landed live or as drafts, undone in one click; plus the stretch band `width` on SECTION/BACKDROP/HERO and a Latin font shelf — `docs/bent-geppetto.md`), before it **v2.55-alpha** (the card gateway: Grow or Cardcom behind one driver — a card order is marked paid only after the provider confirmed it to our server; test mode never looks like money; the keys live in gitignored `config/payments.json`, never the `.pzn` — `docs/bent-store.md` §The card gateway), before it **v2.54-alpha** (a block's class, animation and "hide on mobile" land on every module), before it v2.53-alpha (the store: one flip makes the site a shop — BenTML pages and five storefront modules, the catalog as one `<bent-store>` document with preview/apply/undo and an AI catalog pack, cart → checkout → orders with the server's own quote, and the whole shop inside the `.pzn` backup/hot swap — `docs/bent-store.md`), before it **v2.52-alpha** (the premium tier, second pass: keys are filed per supplier — one stored key used to follow the owner to the next supplier; `gemini` joins with the owner's own AI Studio key; OpenAI's list and field names are current; optional wire fields are revocable on a 400; Claude's cache cuts at the situation's head and gets a moving second breakpoint; the chat shows the meter), before it v2.51-alpha (the premium tier: a price table with dates, a cached briefing prefix, a battery that refuses before it spends and stops at its cap), before it v2.50-alpha, then two PRs that ship nothing (#34: the recommendation by card is DECIDED; #35: the Mac's re-run orders + `mlx-survey.sh variants`) (a thinking model gets the lowest reasoning level it allows and one larger-budget retry, `THOUGHT_OUT` says the rest; the Mac script knows LM Studio cannot load a variant by name; before it: `LOCAL_LLM_WINDOW_CAP` — an MLX row can mean 32K; the Mac survey script finds staff picks, resumes downloads and cannot be lied to about its window; before it: `style` is also a real attribute — the dead-styling door only judges CSS declarations; before it: the human dreams D9–D14 and `--briefs=dreams`; styling that does nothing goes back once, and the briefing says where the look lives), live and deploying from main.
...
| | |
|---|---|
| Package / ROADMAP | **`2.65.0-alpha`** — the Version Log's top row must name it (`smoke-version`) |
| Main tip | v2.65 (the drawer's ✕ belongs to the panel) on top of v2.64 (the Hebrew site through the Bridge; the drawer scrolls) on top of v2.63 (the header moves: four placements, three header shapes, the overlay drawer — CSS only) on top of v2.62 (words that claim a change are not a change) on top of v2.61 (after ✓ the owner is done: the repeat ends the turn, a saved draft never ends red, an unchanged page is not a card, the clock knows a tool call) on top of v2.60 (the repeated closing-turn proposal is dropped) on top of v2.59 (the Bridge rebuild of the English site, the reply budget that follows the page, the cornerstone guide) on top of v2.58 (the English site + the extension courier + the stranger guard + the English track) on top of v2.57 (Geppetto's SVG door + layer names as hints) on top of v2.56 (Geppetto, merged after five review rounds) and v2.55 (the card gateway), v2.54 (class names on every module) and v2.53 (the store) — `git log -1 origin/main` |
| Geppetto (v2.56–57) | `/admin/geppetto` (admin only): a Canva / Figma Sites address, a saved page, **SVG files exported from a design tool** (one per page; "Outline text" must be OFF or the words are vector paths — the screen refuses and says so; pictures are shrunk in the browser because a real export was 154 MB), a Figma file + the owner's token (one request, never stored) or the Figma plugin's JSON → a plan (nothing changes) → preview in its own theme → land live or as drafts → undo. **Layer names are hints** (`src/geppetto/names.js`): a named header becomes the menu even with no links in the file, "Site name" the brand, "Button" a button, network-named icons the social row — never overriding geometry, and all 25 real samples stay byte-identical. Engine `src/geppetto/` (decoders → the puppet → life.js → look.js → land.js). Canva's app export: an element's `A` is its TOP and `B` its LEFT. A Figma **Make** site is code, not a design — refused with that reason |
...
| Local model | provider `local` (a loopback runtime) or `browser` — **Bridge V2 0.5.5**, the extension in `extension-v2a/`. Measured for real since v2.58: `battery-copilot.js --courier=extension` loads the wired Chrome build into a headless Chrome and types the dreams into /admin/chat (`scripts/battery-bridge-courier.js`) |
| The English site (v2.58) | `config.language` turns the site around: new pages ltr, English chrome (`src/site-language.js`, `{{t.*}}` in the layout), an English copilot (`buildCopilotBriefing({ siteLanguage })`), and the settings switch flips every page that read the old way (`pages.flipSiteDirection`; a `meta.lang` page keeps its own). The second Hostinger site is the English showcase — seven pages, a night theme, Grok Imagine pictures, a dreams page generated from the battery JSON (`docs/LOCAL-LLM.md` §5) |
| The Hebrew site (cyan), 2026-09-24 | Rebuilt through the Bridge in Ben's Chrome (v2.63–v2.65, `docs/LOCAL-LLM.md` §5). Ben's corrections the same evening are applied: the old show (ההצגה) and the demos (דוגמאות) are back in the header — **never pull the statistics pages out of the menu again**; a 17-product easter-egg catalog (Shaltiel Industries: game engines, NVIDIA clusters, PLC/Ladder, a dairy product, "תפוז" for sale, the CMS features at ₪0) with the store closed; the names Ben, Paz, Simha, Lilit, whiteno1se and Shaltiel Industries as easter eggs in credits, copy, coupons and the footer; the home page is the poster (Karantina headings, the marquee art behind the hero). Still Ben's call: opening the store, the film-grain effect, deleting the two smoke leftovers. |
| The theme builder's next step (after v2.63) | Ben (2026-09-24): *"more cool custom-made site theme builder … to challenge WordPress' old builders."* v2.63 shipped the shapes (CSS only). **Next, v2.66 — the motion:** `header-scroll="stay\|shrink\|hide\|reveal"` (a bar that compacts after the first screen, or tucks away on the way down and returns on the way up), an optional `scroll-progress` line, `animate` gaining `stagger` for a module's children, a `sticky` flag on a module — all needing the FIRST script a theme ships (≤ 60 lines, reduced-motion aware, exported by `copyThemeAssets` next to `css/main.css`, guarded like an effect). Then `header-layout="split"` (the menu on both sides of the logo), which needs the menu rendered in two halves (the layout has one `<ul>`). |
| Open after v2.61 | (a) the store's storefront strings are still Hebrew on an English site (and the empty-table placeholder `📋 טבלה — הוסיפו שורות` in `src/pzn/table-html.js`); (b) a model that proposes an unchanged page twice costs the owner two tool-call round trips (~2 min each on the 26B for an 80-module page) before the turn ends with the reason — the reads could be cheaper (an edit that names the module to change instead of the whole page); (c) DONE in v2.60/v2.61 — the second card, the locked input after ✓, the red error after a saved draft, the clock's "wrote nothing" during a streamed tool call |
| The battery's guard (v2.58) | LM Studio answers a request for an UNLOADED identifier with whatever is loaded (HTTP 200; only `model` in the reply says so). The relay/extension couriers stop the run when the answering model is not the named one — `lms ps` before a night run, and name the loaded model. Another agent on the Mac (GrokBot) loads models under the survey's names; expect it |
