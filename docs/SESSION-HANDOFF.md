...
| | |
|---|---|
| Package / ROADMAP | **`2.57.0-alpha`** — the Version Log's top row must name it (`smoke-version`) |
| Main tip | v2.57 (Geppetto's SVG door + layer names as hints) on top of v2.56 (Geppetto, merged after five review rounds) and v2.55 (the card gateway), v2.54 (class names on every module) and v2.53 (the store) — `git log -1 origin/main` |
