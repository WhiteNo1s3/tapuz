...
| | |
|---|---|
| Package / ROADMAP | **`2.53.0-alpha`** — the Version Log's top row must name it (`smoke-version`) |
| Main tip | v2.53 (the store) on top of v2.52 — `git log -1 origin/main` |
...
| The store | `src/store/*` (money, settings, catalog, coupons, pricing, orders, pages = the flip, render, document), `src/bentml/store-dialect.js`, `public/tz-store.js`, `src/routes/store-*.js`, `src/injections/store-catalog.js`, `docs/bent-store.md` |
...
| The store takes card payments | **No** — no gateway API is built. The `link` method sends the shopper to the owner's own payment page with the sum and the order number; the owner marks the order paid. No card number or gateway key ever reaches the store |
