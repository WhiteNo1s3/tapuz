...
| | |
|---|---|
| Package / ROADMAP | **`2.60.0-alpha`** — the Version Log's top row must name it (`smoke-version`) |
...
| Open after v2.59 | (a) DONE in v2.60 — a closing-turn proposal identical to the draft just saved is dropped; (b) the turn clock counts `content` deltas only, so while a tool call streams it says "wrote nothing in 45 s"; (c) the store's storefront strings are still Hebrew on an English site |
