...
| Open after v2.59 | (a) after an approved write, the model's closing turn sometimes proposes the same edit AGAIN on a second card (1 of 24 measured approvals, and once live) — the door could compare a closing-turn proposal with the draft it just saved and drop an identical one; (b) the turn clock counts `content` deltas only, so while a tool call streams it says "wrote nothing in 45 s"; (c) the store's storefront strings are still Hebrew on an English site |
