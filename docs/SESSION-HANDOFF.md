...
| | |
|---|---|
| Package / ROADMAP | **`2.51.0-alpha`** — the Version Log's top row must name it (`smoke-version`) |
...
| Premium tier (v2.51) | provider `claude` / `openai` / `xai` / `openrouter` with the owner's key — the CMS fetches it itself. Every chat response carries `spend` (tokens, cache split, money); the price table with its read-date is `src/ai-cost.js`; the Anthropic briefing prefix rides with `cache_control` (a tenth of the input price after the first hop). `docs/CLOUD-LLM.md`, orders in `eval/battery/RUN-CLOUD.md`. **No real key has been run yet** |
