...
**לתפוזיאל אין שלב בנייה בכלל.** אין `npm run build`, ובכוונה — ראו למטה.
`npm run export:static` הוא **ייצוא תוכן**: הוא לוקח את הדפים ה*מפורסמים*
מ-SQLite וכותב HTML לתוך `public/` הקיים. הוא לא מייצר `dist/`, לא `out/`,
ולא `build/`, ואין שום צורך להריץ אותו בפריסה — השרת מרנדר דפים בעצמו.
