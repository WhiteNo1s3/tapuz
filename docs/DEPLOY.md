...
וזו הסיבה שהשגיאה עשויה להופיע **גם אחרי התיקון**.

פלטפורמות עם זיהוי אוטומטי (Hostinger "Deploy Web App", Netlify, Vercel,
Cloudflare Pages) קוראות את `package.json`, רואות סקריפט בשם `build`,
ומסיקות: *"פרויקט עם שלב בנייה → יש תיקיית פלט"*. הן מריצות `npm run build`
ואז מחפשות `dist/` · `build/` · `out/`. תפוזיאל לא מייצר אף אחת מהן — הוא
מייצא HTML לתוך `public/` הקיים.

**התיקון הוא בהגדרות הספק, לא בקוד:** לרוקן את פקודת הבנייה ואת תיקיית
הפלט, ולהצביע על נקודת הכניסה `src/server.js`. ראו §3א.

> `npm run build` בתפוזיאל הוא **ייצוא תוכן**, לא בניית אפליקציה. אין שום
> צורך להריץ אותו בפריסה — השרת מרנדר דפים בעצמו.
