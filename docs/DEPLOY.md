...
### זריעת תוכן עם הפריסה (content seed, v2.13)

ארכיון פריסה יכול לשאת חבילת תוכן חד-פעמית: תיקיית `seed/` עם
`manifest.json` ודפי `.pzn`. בעליית השרת החבילה מוחלת **פעם אחת לכל
מזהה חבילה**, דרך אותם API-ים שהאדמין וגשר הסוכנים משתמשים בהם
(createPage ← savePageSource ← publish ← exportAll) — כל שמירה משאירה
גרסה לשחזור, ודף קיים נדרס רק כשה-manifest אומר `"replace": true`.

חוזה הבטיחות:

- הזריעה חמושה **רק בפריסת נעץ** (כשקיים `‎.tapuz-root‎` — קובץ שקיים
  אך ורק כי מתכון פריסה הזריק אותו). ריצת פיתוח או בדיקה לעולם לא
  זורעת מעצמה; `TAPUZ_SEED=1` מחמש ידנית, `TAPUZ_SEED=0` מנטרל הכול.
- סמן ב-`config/seeded.json` (תחת `TAPUZ_ROOT`) זוכר מה כבר הוחל —
  פריסה חוזרת של אותו ארכיון היא no-op שקט.
- לפני נגיעה ב-`menus.json` / `theme-overrides.json` / `site.json` נשמר
  עותק גיבוי (`<שם>.pre-<מזהה>.json`) לצדם.
- כשל בזריעה נרשם ללוג ומדולג — הוא לעולם לא מפיל את השרת.

חבילה יכולה גם לנקות אחרי קודמותיה — `"cleanup"` ב-manifest:
`{ "backups": "keep-latest", "revisionsKeep": 3 }` משאיר גיבוי אחד (החדש)
לכל קובץ config ולכל ערכת נושא, ומקצץ את היסטוריית הגרסאות של דפי
החבילה ל-N האחרונות. בלי ההנחיה — שום דבר לא נמחק.

בדיקה: `npm run test:seed` (‏`scripts/smoke-seed.js`, הרמטי לגמרי).

### 🔍 "פרסמתי ושום דבר לא השתנה" — טביעת האצבע של הפריסה (v2.24)

בפריסת ארכיון, **push ל-main לא פורס כלום** (אפליקציית ה-GitHub מושעית).
מה שרץ באתר הוא הארכיון האחרון שנבנה — מה-clone המקומי, במצבו באותו רגע.
לפני שבונים ארכיון: `git pull origin main`. ארכיון מ-clone שלא עודכן מעלה
את הקוד הישן בשקט מוחלט, וכל תיקון "לא מגיע לאתר".

איך יודעים מה באמת רץ, בלי לנחש:

1. **בכל דף באתר החי** יש `<meta name="generator" content="Tapuziel <גרסה>+<חותמת>[.<sha>]">`
   (view-source). החותמת נכתבת בזמן הבנייה ל-`.build-id` (‏`npm run build`).
   `+dev` = לא עבר בנייה (clone שהופעל ב-`npm start`).
2. **בעמוד ערכת הנושא** (`/admin/theme`, מתחת לתצוגה המקדימה) מודפסים: מזהה
   הבנייה, `site root` (התיקייה שהאפליקציה באמת משתמשת בה — ‎`TAPUZ_ROOT`‎/הנעץ),
   מועד הבנייה האחרון של האתר הסטטי, וגרסת גיליון ה-CSS (`css v…`).
3. **בדף החי** קישור הגיליון הוא `/css/main.css?v=<גרסה>` — הגרסה משתנה עם כל
   שינוי בערכת הנושא. אם ה-`v` בדף החי שווה ל-`css v` בעמוד ערכת הנושא — הצינור
   שלם, ומה שנשאר הוא cache של הדפדפן (רענון קשה / חלון פרטי).

מפת תקלות: אין `meta generator` בכלל → קוד ישן, בנו ארכיון מ-clone מעודכן.
יש meta אבל אין `?v=` → הדף הוגש מ-`public_html` (שכבה סטטית שמאפילה על
האפליקציה — ראו ניקוי ה-shadow ב-`scripts/deploy-decoy.js`). יש `?v=` אבל
הצבעים ישנים → פתחו את `/css/main.css?v=…` ישירות; אם הוא נכון, זה cache.
...
> אז קודם תקנו את הגדרות הפריסה (Express, בלי בנייה, `src/server.js`).
