...
| First HTTP (**after lockout reset + owner wizard**) | homepage **200**, `Last-Modified: Mon, 21 Sep 2026 22:07:10 GMT`, generator `2.52.0-alpha+20260921-220706Z.ee117e5` (PR #40 auto-deploy on `main`; this reset did not restart or env-replace cyan) | Owner username `benshaltiel`; `GET /` **200** title `TINKERiNG TApuZ`; `/admin/create-account` 302 → login |
...
**Current owner setup: done.** Owner CMS username: `benshaltiel`. `GET /admin/create-account` 302 → `/admin/login`. Wizard + product **בנה את האתר**: `GET /` **200**, title `TINKERiNG TApuZ` (דף הבית / אודות / צור קשר / מאמרים). Authenticated `GET /admin/db/export.pzn` (SQLite `application_id` `TPUZ`) and `GET /admin/api/site-package/export` (`format: tapuz-site`) succeeded. `/admin/ai-setup` 200 with Bridge V2 Chrome ZIP. The seed-dev-admin script **refuses** `NODE_ENV=production` and was not run. No password is recorded here.
