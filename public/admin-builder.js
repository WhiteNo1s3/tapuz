...
(function () {
  ...
  var MODULES = [
    ...
    { type: 'code', label: 'בלוק קוד', hint: 'גדר קוד', icon: '‹›', group: 'תוכן', keyword: 'CODE' },
    { type: 'tags', label: 'תגיות', hint: 'שבבי תגיות', icon: '#', group: 'מבנה', keyword: 'TAGS' },
  ...
  function renderBlockBody(block) {
    ...
    if (block.type === 'author') {
      wrap.innerHTML =
        ...
        (d.role || d.time ? '<div class="preview-author-meta" style="color:#78716c;font-size:.85rem">' + esc([d.role, d.time].filter(Boolean).join(' · ')) + '</div>' : '') +
