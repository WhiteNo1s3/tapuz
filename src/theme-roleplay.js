...
const SKELETON = [
  ...
  ['body.menu-side', 'האתר במצב מסילה צדית (menu="side" / "side-end") — התפריט אנכי'],
  ['body.menu-bottom', 'התפריט כבר תחתון קבוע, כמו באפליקציה (menu="bottom")'],
  ['body.header-centered', 'הלוגו מעל תפריט ממורכז (header-layout="centered")'],
  ['body.header-floating', 'הכותרת כגלולה צפה — מנותקת, מעוגלת, זכוכית (header-layout="floating")'],
  ['body.nav-overlay', 'המגירה ☰ נפתחת כפאנל מעל הדף (drawer="overlay")'],
...
function knobsDoc() {
  return [
    ...
    '  <bent-layout max-width="900px|1100px|1200px" menu="top|side|side-end|bottom" header-width="content|wide|full" />',
    ...
    '- `bent-chrome`: ערך ריק (`""`) = ברירת המחדל של הערכה. `header-bg` כהה מחייב `header-text` בהיר. **התפריט**: `menu="side"` ב-`bent-layout` = מסילה אנכית (מתאימה ל-8+ פריטים); `header-width` = כמה רחבה הכותרת (`wide` = 1140px, `full` = מקצה לקצה); `menu-overflow` = מה קורה לתפריט ארוך: `wrap` נשבר לשורה שנייה · `scroll` רצועה אחת שנגללת · `drawer` כפתור ☰ בכל רוחב; `menu-align` יישור; `menu-gap`/`menu-size` צפיפות וגודל; `menu-fold` = כמה פריטים עליונים לפני קיפול "עוד" (0 = בלי; מתאים ל-8+ פריטים); `menu-collapse` = מאיזה רוחב מסך התפריט הופך למגירה ☰ (`sm` 560px · `md` 720px · `lg` 1024px · `never`); `menu-current` = איך הדף הנוכחי מסומן (קו תחתון · גלולה · מודגש · בלי). תפריטי משנה נפתחים בריחוף/מקלדת מעצמם. **צורת הכותרת** (v2.63): `header-layout` = `bar` לוגו בצד אחד ותפריט בצד השני (ברירת מחדל) · `centered` לוגו מעל תפריט ממורכז (מאסטהד של מגזין) · `floating` גלולה צפה — כותרת מנותקת, מעוגלת, מזכוכית, שמרחפת מעל הדף (מודרני; מתאים ל-3–6 פריטים); `drawer` = מה ☰ פותח: `inline` הרשימה בתוך הכותרת · `overlay` פאנל צד מעל הדף; ב-`bent-layout`: `menu="side-end"` = מסילה בצד השני · `menu="bottom"` = בר תחתון קבוע כמו באפליקציה (עד 6 פריטים, בלי ☰).',
