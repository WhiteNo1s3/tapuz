...
function decompileBlock(block, indent) {
  ...
  switch (type) {
    ...
    case 'spacer': {
      ...
      if (size === 'md') return `${pad}SPACE`;
      return `${pad}SPACE(size: ${size})`;
    }
    case 'divider': {
      const st = d.bentStyle || (d.style === 'dashed' ? 'dots' : 'line');
      if (st === 'line') return `${pad}DIVIDER`;
      return `${pad}DIVIDER(style: ${st})`;
    }
    case 'list': {
      const params = [];
    ...
    case 'stats':
      return `${pad}STATS(columns: ${d.columns || 3})`;
    case 'logos':
      return `${pad}LOGOS`;
    case 'faq':
      return `${pad}FAQ`;
