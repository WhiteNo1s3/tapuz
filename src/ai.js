...
/**
 * @param {{ provider?: string, model?: string, apiKey?: string }} patch
...
async function generate({ system = '', user = '', history = [] } = {}) {
  const s = load();
  const key = String(s.apiKey || '');
  if (!key) throw new Error('לא הוגדר מפתח API — הגדירו אותו בצ׳אט (ההגדרות בצד)');
  ...
  const model = (provider.models || []).includes(s.model) ? s.model : provider.defaultModel;
