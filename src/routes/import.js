...
router.get('/admin/import', (req, res) => {
  const soon = [
    ...
    ['Mobeart', 'ייצוא Mobeart']
      ...
      <section class="card tight is-soon">
        <div class="card-head"><span class="ico">📦</span>${v[0]}<span class="pill" style="margin-inline-start:auto">בקרוב</span></div>
        <p class="faint" style="margin:0">${v[1]} — נוסף בקרוב.</p>
      </section>`).join('');
  const html = `
    ...
    <div class="container page-body" style="max-width:920px">
      ...
      <section class="card lead-card">
        ...
        <div class="row" style="flex-wrap:nowrap;gap:12px">
          <input type="file" id="wp-file" accept=".xml,.wxr" class="input" style="flex:1;min-width:0">
          <label class="check-row" style="margin-bottom:0;white-space:nowrap"><input type="checkbox" id="wp-publish"> פרסם מיד</label>
      ...
      <div class="side-title">מערכות נוספות</div>
      ${soon}
    </div>
    <script>
    (function(){
        ...
        btn.addEventListener('click', function(){
          ...
          reader.onload = function(){
            ...
            .then(function(d){
              ...
              out.innerHTML = '<div class="ok-text" style="margin-bottom:6px">יובאו '+d.count+' דפים:</div><ul style="margin:0;padding-inline-start:18px">'+li+'</ul>';
            })
